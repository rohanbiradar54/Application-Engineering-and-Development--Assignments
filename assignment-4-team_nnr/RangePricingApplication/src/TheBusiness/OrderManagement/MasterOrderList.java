...
return sum;
  ...
  public double getSalesPerformanceForSalesPerson(SalesPersonProfile salesPersonProfile) {
        double salesPerformance = 0;

        for (Order order : orders) {
            if (order.getSalesperson().equals(salesPersonProfile)) {
                salesPerformance += order.getOrderTotal();
            }
        }
